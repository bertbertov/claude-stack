...
## Companion skills (use the right tool per image type)

| Image type | Use this skill |
|---|---|
| Hero / artistic / cinematic (default) | this skill (frontendwebsiteimageskill) + Flux Krea Dev FP8 locally |
| Image with heavy readable typography (menus, infographics, posters with text) | **gpt-image-2** — only model with 99% text rendering |
| Hosted alternative when local ComfyUI is busy | **prompt-images** + **run-models** + **find-models** + **compare-models** (Replicate) |
| Likeness preserved (caricature / portrait covers) | Phoenix img2img + **prompt-images** for prompt patterns |
| Outfit-swap / inpaint on people | Juggernaut XL Inpaint OR **prompt-images** Replicate variants |
